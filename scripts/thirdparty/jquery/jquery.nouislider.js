...
(function ($) {
	...
	$.fn.noUiSlider = function (options, flag) {
		...
		//wrap arround
		var wrapper = jQuery('<div class="slider-container"></div>');
						...
						handles[i].find('div').on(onEvent, function (e) {
							...
							if (!slider.hasClass('disabled')) {
								...
								$(document).on(moveEvent, function (f) {
									...
									call(settings.slideStop, slider);
				...
				if (typeof arguments[0] !== 'undefined') {
					...
					return this.each(function () {
						...
						for (var i = 0; i < setup.handles.length; i++) {
							if (val[i] != null) {
								var proposal = correct(percentage.to(setup.data.range, val[i]), $(this), setup.handles[i]);
								setup.handles[i].css(setup.pos, proposal + '%').data('input').val(percentage.is(setup.data.range, proposal).toFixed(setup.res));
							}
						}
					});
